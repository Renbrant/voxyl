      ...
      <div className="px-4 mt-4">
        <div className="flex items-center justify-between mb-3">
          <h2 className="font-semibold text-foreground flex items-center gap-2">
            ...
            {loadingEps && episodes.length > 0 && <span className="flex items-center gap-1 text-xs px-2 py-1 rounded-full bg-muted text-muted-foreground font-medium"><Loader2 size={11} className="animate-spin" />{t('detailLoading')}</span>}
            {!loadingEps && backgroundSyncSource === 'local' && episodes.length > 0 && <span className="text-xs px-2 py-1 rounded-full bg-accent/10 text-accent font-medium">{t('detailCached')}</span>}
