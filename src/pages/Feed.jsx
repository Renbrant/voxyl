      ...
      <div className="px-4 pb-24">
        ...
        {topPodcasts.length > 0 && (
          <div>
            ...
            {/* Hero Podcast */}
            {topPodcasts[0] && (
              <motion.div initial={{ opacity: 0, y: 16 }} animate={{ opacity: 1, y: 0 }} className="mb-3 p-3 rounded-2xl border border-border bg-card">
                <div className="flex gap-3">
                  {topPodcasts[0].image && (
                    <img src={topPodcasts[0].image} alt="" className="w-20 h-20 rounded-lg object-cover flex-shrink-0" />
                  )}
                  <div className="flex-1 min-w-0">
                    <p className="text-sm font-semibold text-foreground line-clamp-1">{topPodcasts[0].title}</p>
                    <p className="text-xs text-muted-foreground line-clamp-1">{topPodcasts[0].author}</p>
                    <p className="text-xs text-primary mt-1">❤️ {topPodcasts[0].likeCount} favoritos</p>
                  </div>
                </div>
              </motion.div>
            )}
